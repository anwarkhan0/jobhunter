...
const availableSites = [
  {
    id: "expatriates",
    name: "Expatriates",
    domain: "https://www.expatriates.com",
  },
  ...
  { id: "bayt", name: "Bayt", domain: "https://www.bayt.com/" },
  {
    id: "naukarigulf",
    name: "NaukariGulf",
    domain: "https://www.naukrigulf.com/",
  },
  {
    id: "gulftalent",
    name: "Gulftalent",
    domain: "https://www.gulftalent.com/",
  },
];
...
export default function JobSearchApp() {
  ...
  const [rawResults, setRawResults] = useState<JobSite[]>([]);
  ...
  const filterResults = (allResults: JobSite[], query: string) => {
    ...
    return allResults
      ...
      .filter((site) => site.jobs.length > 0);
  };

  const handleFilterChange = (value: string) => {
    setFilterKeyword(value);
    setResults(filterResults(rawResults, value));
  };
    ...
    try {
      ...
      while (true) {
        ...
        if (Array.isArray(data.result)) {
          ...
          const mappedResults = lastResult.map((site: any) => ({
            ...
            name:
              ...
              site.website
                ...
                .slice(1),
            domain: site.website,
            jobs: Array.isArray(site.data)
              ? site.data.map((job: any, idx: number) => ({
          ...
          setResults(filterResults(mappedResults, filterKeyword));
        }
  ...
  const totalJobs = Array.isArray(results)
    ? results.reduce(
        (sum, site) => sum + (Array.isArray(site.jobs) ? site.jobs.length : 0),
        0,
      )
    : 0;
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center mb-8">
          <h1 className="text-4xl font-bold text-gray-900 mb-2">
            Job Hunt in Saudi Arabia
          </h1>
          <p className="text-lg text-gray-600">
            Search multiple job sites at once and download results
          </p>
        </div>
        ...
        <Card className="mb-8">
          <CardHeader>
            ...
            <CardDescription>
              Select job sites and enter your search criteria
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            ...
            {/* Search Input */}
            <div>
              {/* <h3 className="text-sm font-medium mb-3">Search Keywords</h3> */}
              {/* <Input
            ...
            {results.length > 0 ? (
              <div>
                ...
                <Input
                  placeholder="Filter displayed jobs by keyword"
                  value={filterKeyword}
                  onChange={(e) => handleFilterChange(e.target.value)}
                />
                <p className="text-sm text-gray-500 mt-1">
                  Filter titles, companies, locations, and descriptions in
                  current results.
                </p>
              </div>
            ) : (
              ...
              <div className="flex items-center justify-between">
                <div>
                  <CardTitle className="flex items-center gap-2">
                    Search Results
                    {totalJobs > 0 && (
                      <Badge variant="secondary">{totalJobs} jobs found</Badge>
                    )}
                  </CardTitle>
                  <CardDescription>
                    {isLoading
                      ? "Searching job sites..."
                      : `Results from ${results.length} sites`}
                  </CardDescription>
                </div>
                {results.length > 0 && (
                  <div className="flex gap-2">
                    ...
                    <div key={site.id}>
                      ...
                      <div className="flex items-center gap-3 mb-4">
                        ...
                        <Badge variant="outline">
                          {Array.isArray(site.jobs) ? site.jobs.length : 0} jobs
                        </Badge>
